...
void surf (Input IN, inout SurfaceOutput o) {
	...
	float3 sphere2 = 1 - saturate(dis2 / _Radius);

	float3 sphereNoise2 = noisetexture.r * sphere2;

	float3 DissolveLine2 = step(sphereNoise2 - _DisLineWidth, _DisAmount) * step(_DisAmount, sphereNoise); // line between two textures
	DissolveLine *= _DisLineColor; // color the line

	float3 primaryTex2 = (step(sphereNoise2 - _DisLineWidth, _DisAmount) * c.rgb);
	float3 secondaryTex2 = (step(_DisAmount, sphereNoise2) * c2.rgb);
	float3 resultTex2 = primaryTex + secondaryTex2 + DissolveLine2;

	o.Albedo = resultTex + resultTex2;
