Shader "Custom/RedEyesFloorTransistionShader" {
	Properties{
...
#pragma target 3.0
	...
	void surf(Input IN, inout SurfaceOutputStandard o) {
		fixed4 c = tex2D(_SecTex, IN.uv_MainTex) * _Color;
		half dist = distance(_GLOBALMaskPosition, IN.worldPos);
		half sphere = 1 - (dist - _GLOBALMaskRadius);
		if (dist > _GLOBALMaskSoftness)
		{

			c = tex2D(_MainTex, IN.uv_MainTex) * _SecColor;
		}
		//clip(sphere - 0.1);
