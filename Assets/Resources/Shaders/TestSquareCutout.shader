Shader "Custom/TestSquareCutout" {
	...
	SubShader {
		...
		void surf (Input IN, inout SurfaceOutputStandard o) 
		{
			...
			//float3 dis = distance(_PositionA, _PositionB);


			//float3 sphereNoise = noisetexture.r * dis;

			//float3 DissolveLine = step(sphereNoise, _DisAmount) * step(_DisAmount, sphereNoise); // line between two textures
			...
			float dis3 = distance(dis, dis2);
			float3 sphere = dis3 / _Radius;

			float3 dis4 = distance(RightPoint, IN.worldPos);
			float3 dis5 = distance(LeftPoint, IN.worldPos);
			float dis6 = distance(dis, dis2);
			float3 sphere2 = dis6 / _Radius;

			float3 sphereNoise = noisetexture.r * sphere;

			float3 sphereNoise2 = noisetexture.r * sphere2;
			...
			float3 resultTex = primaryTex;//+ secondaryTex;//+ DissolveLine + DissolveLine2;
			...
			{
				o.Albedo = resultTex + DissolveLine;
			}
