Shader "Toon/Lit Metal Mask" {
	Properties {
		_Color ("Base Color", Color) = (0.8,0.4,0.15,1)
		_MainTex ("Base (RGB)", 2D) = "white" {}
		_Ramp ("Toon Ramp (RGB)", 2D) = "white" {} 
		[Header(Metal)]
		_Mask ("Mask R = spec", 2D) = "black" {} 
...
void surf (Input IN, inout SurfaceOutput o) {
	half4 c = tex2D(_MainTex, IN.uv_MainTex); // main texture
	half4 m = tex2D(_Mask, IN.uv_MainTex);// mask texture
	...
	float3 highlightAlbedo =  (step(_HighlightOffset,highlight) * _HiColor); //glowing highlight
	o.Albedo = (c.rgb * (1-m.r)) + ((baseAlbedo + specularAlbedo  + highlightAlbedo) * m.r);// result
